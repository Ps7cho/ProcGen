/// @description scr_button_set_on_mouseout_cb(button_instance, script_asset, [arguments_as_array]);
...
with (o) {
    if (is_string(scr_in)) {
        ...
        scr_button_unset_on_mouseout_cb(self);
        return undefined;
    }
    
    onMouseOutCallbackScript = scr;
    if (argument_count > 2) {
        if (is_array(argument[2])) _oMOutargs = argument[2];
        else _oMOutargs = undefined; // to clean up
    }
