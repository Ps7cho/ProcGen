/// @description  scr_cbDataGUI_unset_tooltip(target_object, target_variable);
/// @param target_object
/// @param  target_variable
/**
* @brief  Removes the tooltip you set to a cbDataGUI row.
*
* @param  {instance} target_obj          An object instance to be targeted
* @param  {string}   target_variable_str An instance variable name as a string. 
*                                        (Eg. If you want to change obj.radius, 
*                                        use "radius")
*         
* @return {instance} Returns true if successful. Returns undefined if fails.
*
* @see scr_cbDataGUI_set_tooltip
*/

return _scr_cbDataGUI_tooltip_unset(argument0, argument1);

